...
const EntryBlock = ({ entry }: EntryBlockProps) => {
    ...
    <Card>
      <CardContent className="p-5">
        <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-400">
          <span>
            <span className="font-semibold text-zinc-200">{summary.entries}</span>{" "}
            entries
          </span>
          <span className="text-zinc-600">|</span>
          <span>
            <span className="font-semibold text-zinc-200">{summary.users}</span>{" "}
            user
          </span>
          <span>
            <span className="font-semibold text-zinc-200">{summary.assistants}</span>{" "}
            assistant
          </span>
          <span className="text-zinc-600">|</span>
        ...
        <div className="flex flex-col gap-3">
          {transcript.entries.map((entry) => (
            <EntryBlock key={entry.index} entry={entry} />
          ))}
        </div>
      </CardContent>
    </Card>
